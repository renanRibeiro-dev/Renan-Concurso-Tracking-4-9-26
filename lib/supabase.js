...
export async function salvarQuestao({ subtopico_id, caderno, numero, resultado, miniassunto }) {
  const {
    data: { user },
  ...
  const { data, error } = await supabase
    .from("questoes_v2")
    .insert([{ subtopico_id, caderno, numero, resultado, miniassunto: miniassunto || null, user_id: user.id }]);
