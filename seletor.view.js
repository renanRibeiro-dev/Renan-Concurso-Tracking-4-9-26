...
export function mount(container, { onSelect, onClose }) {
  ...
  function subtopicoCompleto(s) {
    if (s.miniassuntos && s.miniassuntos.length > 0) {
      return s.miniassuntos.every((m) => m.estudado);
    }
    return questoes.some((q) => q.subtopico_id === s.id);
  }

  function materiaCompleta(materia) {
    const subs = subtopicos.filter((s) => s.materia === materia);
    return subs.length > 0 && subs.every((s) => subtopicoCompleto(s));
  }
    ...
    if (items.length === 0) {
      ...
      items.forEach((item) => {
        ...
        if (state.level === 3) {
          ...
          if (subtopicoCompleto(item)) row.classList.add("seletor-item--escurecido");
         } else if (state.level === 2) {
          row.textContent = item;
          row.classList.add(MATERIA_QUENTE.has(item.trim()) ? "seletor-item--quente" : "seletor-item--frio");
          if (materiaCompleta(item)) row.classList.add("seletor-item--escurecido");
        } else if (state.level === 4) {
  ...
  Promise.all([
    ...
    getAppData("questoes_v2"),
  ])
  .then(([subtopicosData, questoesData]) => {
    subtopicos = Array.isArray(subtopicosData) ? subtopicosData : [];
    questoes = Array.isArray(questoesData) ? questoesData : [];
    render();
  })
  .catch((err) => {
    console.error("Erro ao buscar subtopicos/questoes:", err);
  });
}
